...
#pragma warning restore 0219, 414
	...
	protected virtual void DrawCreateAnimatorController(string gameObjectName)
	{
		...
		{
			string directoryPath = string.Format("Assets/#_Specific/#Animator Controllers/#{0}", gameObjectName);

			DirectoryInfo directoryInfo = Directory.CreateDirectory(directoryPath);

			string controllerPath = string.Format("{0}/{1}.controller", directoryPath, gameObjectName);
			...
			if (controller == null)
			{
				...
				string animationClipFormatPath = Path.Combine(directoryPath, "{1}.anim");

				// Create and assign Show animation clip
				AnimationClip animationClipShow = AssetDatabase.LoadAssetAtPath<AnimationClip>(string.Format(animationClipFormatPath, gameObjectName, gameObjectName + " Show Animation"));
				...
				if (animationClipShow == null)
				{
					...
					AssetDatabase.CreateAsset(animationClipShow, string.Format(animationClipFormatPath, gameObjectName, animationClipShow.name));
				}

				stateShown.motion = animationClipShow;

				// Create and assign Hidden animation clip
				AnimationClip animationClipHidden = AssetDatabase.LoadAssetAtPath<AnimationClip>(string.Format(animationClipFormatPath, gameObjectName, gameObjectName + " Hide Animation"));
				...
				if (animationClipHidden == null)
				{
					...
					AssetDatabase.CreateAsset(animationClipHidden, string.Format(animationClipFormatPath, gameObjectName, animationClipHidden.name));
				}
